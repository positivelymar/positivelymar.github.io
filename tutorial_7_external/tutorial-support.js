...
export function handleChartError(error) {
    ...
    if (canvas) {
        ...
        ctx.fillStyle = '#f9f3db';
        ctx.fillRect(0, 0, canvas.width, canvas.height);
        
        ctx.fillStyle = '#0f4a42';
        ctx.font = '16px Georgia';
        ctx.textAlign = 'center';
        ctx.fillText('Chart could not be made', canvas.width/2, canvas.height/2 - 10);
        ctx.fillText('Check console, please!', canvas.width/2, canvas.height/2 + 15);
    }
}
