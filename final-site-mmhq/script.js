...
function assignZone(parkName) {
    const map = {
        ...
        'Barra Honda National Park':    'tempisque',
...
function initMap() {
    const bounds = L.latLngBounds([5.4, -87.2], [12.5, -80.5]);
...
el.addEventListener('mouseenter', () => {
        if (!el.classList.contains('is-selected')) marker.openPopup();
    });

    el.addEventListener('mouseleave', () => {
        if (!el.classList.contains('is-selected')) {
            const selectedEl = el.closest('.panel-spots')?.querySelector('.is-selected');
            if (selectedEl) selectedEl._marker.openPopup();
            else map.closePopup();
        }
    });
    ...
    el.addEventListener('click', () => {
        ...
        el.closest('.panel-spots')?.querySelectorAll('.spot-item-clickable').forEach(item => {
...
function initButtonListeners(parks, spots) {
    document.addEventListener('click', e => { 
        ...
        if (e.target.closest('.panel') && !e.target.closest('.spot-item-clickable') && map) map.closePopup();
...
function resetButtons() {
    activeButtons.stay  = false;
    activeButtons.parks = false;
    document.querySelectorAll('.btn-stay, .btn-np').forEach(btn => btn.classList.remove('active'));
...
function initIntroAnimations() {
    ...
    document.querySelectorAll('.panel-intro').forEach(section => {
        ...
        .from(title, { y: 30, opacity: 0, duration: 0.9, ease: 'power2.out' })
        .from(text,  { y: 20, opacity: 0, duration: 0.8, ease: 'power2.out' }, '-=0.4');
    });

    gsap.to('.hero-bg', {
        scale: 1.1, y: 80, ease: 'none',
